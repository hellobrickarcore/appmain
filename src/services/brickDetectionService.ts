...
export class DetectionStabilizer {
  ...
  constructor(_persistenceWindowMs = 8500, smoothingFactor = 0.04, lockThreshold = 100) {
...
export const detectBricks = async (
  ...
  const { 
    ...
    captureInProgress: _captureInProgress = false
  } = options;

  // Use _captureInProgress to satisfy lint
  if (_captureInProgress) {
    console.log('[BrickDetection] Capture in progress...');
  }
  ...
  try {
    ...
    // No filters — raw camera data preserves all brick colors including white
    ...
    if (!_responseText.trim().startsWith('{')) {
      throw new Error(`Non-JSON response (starts with ${_responseText.trim().slice(0, 10)})`);
    }
    ...
    if (error.name === 'AbortError') _aborted = true;
    
    console.error('🔍 SCANNER ERROR DIAGNOSTICS:', {
      stage,
      requestUrl: CONFIG.DETECT_IMAGE,
      status: _status,
      responseText: _responseText.slice(0, 100), 
      aborted: _aborted,
      detectorState,
