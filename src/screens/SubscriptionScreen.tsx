...
export const SubscriptionScreen: React.FC<SubscriptionScreenProps> = ({ onNavigate }) => {
  ...
  const weeklyPackage: PurchasesPackage | null = offering?.weekly ?? null;

  // Pricing display (dynamic from RC, with fallbacks)
  const yearlyPrice = yearlyPackage?.product?.priceString ?? '$39.99';
  const weeklyPrice = weeklyPackage?.product?.priceString ?? '$9.99';

  // Calculate savings percentage
  const yearlyCost = yearlyPackage?.product?.price ?? 39.99;
  const weeklyCost = weeklyPackage?.product?.price ?? 9.99;
  const weeklyAnnualCost = weeklyCost * 52;
  const savingsPercent = weeklyAnnualCost > 0
    ? Math.round(((weeklyAnnualCost - yearlyCost) / weeklyAnnualCost) * 100)
    : 75;
  ...
  const handlePurchase = async (pkg?: PurchasesPackage | null) => {
    const packageToPurchase = pkg ?? (selectedPlan === 'yearly' ? yearlyPackage : weeklyPackage);
    ...
    <div className="fixed inset-0 bg-[#0A0A14] text-white z-50 flex flex-col font-sans overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-6 pt-[max(env(safe-area-inset-top),3rem)] pb-4">
        ...
        <div className="space-y-3 mb-6">
          ...
          >
            <span className="text-white font-bold text-[15px]">Weekly</span>
            <span className="text-white font-black text-[17px]">{weeklyPrice}</span>
          </button>
        </div>
